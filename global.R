...
locales_es <- c("es_CO.UTF-8", "es_ES.UTF-8", "Spanish_Spain.1252", "es_ES")
for (.cat in c("LC_TIME", "LC_MONETARY", "LC_MESSAGES")) {
  .fijado <- FALSE
  for (.loc in locales_es) {
    .res <- suppressWarnings(
      tryCatch(Sys.setlocale(.cat, .loc), error = function(e) "")
    )
    if (nzchar(.res) && !grepl("^(C|POSIX)$", .res)) {
      .fijado <- TRUE
      break
    }
  }
  if (verbose) {
    if (.fijado) {
      message(sprintf("[OK]   %s = %s", .cat, Sys.getlocale(.cat)))
    } else {
      message(sprintf("[WARN] %s: ningún locale es_* disponible en el SO.", .cat))
      message("       Los nombres de mes saldrán en inglés.")
      message("       Solución: sudo locale-gen es_CO.UTF-8 && sudo update-locale")
    }
  }
}
rm(.cat, .loc, .res, .fijado, locales_es)
...
racafeCore::Loadpkg(c("shiny", "bs4Dash", "shinyBS", "shinyjs",
                      "shinyWidgets", "tidyverse", "reactable",  "scales", 
                      "plotly",  "rlang", "waiter", "glue", "lubridate", 
                      "stringr", "purrr", "sf", "leaflet"))

# Impresiones ----
tit_app <- "Informes de Gestión"
...
## Geografia ----
geo_dpto       <- readRDS("/home/compartido/data/geo_col_dpto_mgn2025.rds")
geo_mun        <- readRDS("/home/compartido/data/geo_col_mun_mgn2025.rds")
geo_centroides <- readRDS("/home/compartido/data/CentroidesDeptos.rds")
GeoTrilladoras <- readRDS("/home/compartido/data/GeoTrilladoras.rds")
...
# Constantes ----
fecha_ej <- PrimerDia(Sys.Date())- months(1)
# Sources ----
